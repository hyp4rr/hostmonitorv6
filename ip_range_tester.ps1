# IP Range Tester for Windows - Logs to CSV
...
# Initialize CSV with additional IP columns and DNS flush status
Write-Host "Initializing log file: $LogFile..." -ForegroundColor Yellow
try {
    $csvHeader = "Timestamp,IP,Gateway_Ping,Internet_Ping,Gateway_MS,DNS_Flush_Status"
...
try {
    for ($i = 24; $i -le 27; $i++) {
        for ($j = 1; $j -le 254; $j++) {
            ...
            Write-Progress -Activity "Testing IP Range" -Status "Testing $ip ($totalIPs/1016)" -PercentComplete (($totalIPs / 1016) * 100)
            ...
            try {
                ...
                # Flush DNS cache with verification
                $dnsFlushStatus = "Success"
                try {
                    Clear-DnsClientCache -ErrorAction Stop
                    Start-Sleep -Milliseconds 300
                    $dnsFlushStatus = "Success"
                } catch {
                    $dnsFlushStatus = "Failed: $($_.Exception.Message)"
                    Write-Warning "DNS flush failed for $ip : $($_.Exception.Message)"
                }
                ...
                $logLine = "$timestamp,$ip,$gatewayStatus,$internetStatus,$gatewayMS,$dnsFlushStatus"
                ...
                Write-Host "$ip - GW: $gatewayStatus ($gatewayMS ms) | Internet: $internetStatus | DNS Flush: $dnsFlushStatus$additionalStatusText" -ForegroundColor $color
                ...
                $logLine = "$timestamp,$ip,Error,Error,N/A,Error"
                foreach ($testIP in $additionalIPs) {
                    $logLine += ",Error,N/A"
                }
                $logLine += ",$errorMsg"
    ...
    Write-Host "Total IPs in Range: $totalIPs" -ForegroundColor White
