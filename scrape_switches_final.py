...
# Find all input buttons - look for the pattern more carefully
# The onclick contains showInfo with device info
button_pattern = r'<input\s+type=button[^>]*onclick="[^"]*showInfo[^"]*"[^>]*>'

# Find all device buttons
button_matches = re.findall(button_pattern, html_content, re.IGNORECASE | re.DOTALL)

# If that doesn't work, try a simpler approach - find all input buttons
...
for button_html in button_matches:
    ...
    # Extract onclick - need to handle escaped quotes
    ...
    if onclick and 'showInfo' in onclick:
        # Extract the two parameters from showInfo('param1','param2')
        # Handle both single and double quotes
        showinfo_match = re.search(r"showInfo\(['\"]([^'\"]*)['\"],\s*['\"]([^'\"]*)['\"]", onclick, re.IGNORECASE)
        if not showinfo_match:
        ...
        if showinfo_match:
            ...
            # Look for IP in info content
            # Try "Ping IP" or "Ping: IP" 
            ip_match = re.search(r'Ping[:\s]+' + ip_pattern, info_content, re.IGNORECASE)
            if ip_match:
                ip_address = ip_match.group(1)
    
    # Fallback: extract from value/title
    if not device_name:
...
# Write to CSV
output_file = 'switches_extracted_final.csv'
